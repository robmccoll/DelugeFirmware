...
keepSearchingForward:
    	...
    	int endIThisModControllable = backedUpParamManagers.search((uintptr_t)modControllable + 4, GREATER_OR_EQUAL, searchedUpToAndIncluding + 1); // Search just by first word
...
ParamManager* Song::getBackedUpParamManagerPreferablyWithClip(ModControllableAudio* modControllable, Clip* clip, ParamManager* stealInto) {
	...
	int iAnyClip = backedUpParamManagers.search((uintptr_t)modControllable, GREATER_OR_EQUAL); // Search just by first word
	...
	while (i < backedUpParamManagers.getNumElements()) {
		...
		if (backedUp->clip == clip) {
			...
			// Othwerwise...
				...
				int j = backedUpParamManagers.search((uintptr_t)modControllable, GREATER_OR_EQUAL, 0, i); // Search by first word only
...
void Song::deleteBackedUpParamManagersForModControllable(ModControllableAudio* modControllable) {
	...
	int iAnyClip = backedUpParamManagers.search((uintptr_t)modControllable, GREATER_OR_EQUAL); // Search by first word only
