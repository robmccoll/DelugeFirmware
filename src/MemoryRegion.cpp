...
void MemoryRegion::setup(void* emptySpacesMemory, int emptySpacesMemorySize, uintptr_t regionBegin,
                         uintptr_t regionEnd) {
...
void MemoryRegion::verifyMemoryNotFree(void* address, uint32_t spaceSize) {
	for (int i = 0; i < emptySpaces.getNumElements(); i++) {
		...
		else if (emptySpaceRecord->address <= (uintptr_t)address
		         && (emptySpaceRecord->address + emptySpaceRecord->length > (uintptr_t)address)) {
			Uart::println("free mem overlap on left!");
			numericDriver.freezeWithError("dddd");
		}
		else if ((uintptr_t)address <= (uintptr_t)emptySpaceRecord->address
		         && ((uintptr_t)address + spaceSize > emptySpaceRecord->address)) {
...
goingToReplaceOldRecord:
		int i = emptySpaces.searchMultiWordExact((uintptr_t*)recordToMergeWith, &insertRangeBegin,
...
uint32_t MemoryRegion::shortenRight(void* address, uint32_t newSize) {
	...
	uint32_t* __restrict__ lookRight = (uint32_t*)((uintptr_t)address + oldAllocatedSize
...
uint32_t MemoryRegion::shortenLeft(void* address, uint32_t amountToShorten, uint32_t numBytesToMoveRightIfSuccessful) {
	...
	uint32_t* __restrict__ lookLeft =
	    (uint32_t*)((uintptr_t)address - 8); // Looking to what's directly left of our old allocated space
...
tryNotStealingFirst:
		...
		for (int tryingStealingYet = 0; tryingStealingYet < 2; tryingStealingYet++) {
			...
			for (int lookingLeft = 0; lookingLeft < 2; lookingLeft++) {
				...
				uintptr_t spaceHereAddress =
				    lookingLeft ? (uintptr_t)lookLeft - emptySpaceHereSizeWithoutHeaders : (uintptr_t)(lookRight + 1);
...
void MemoryRegion::extend(void* address, uint32_t minAmountToExtend, uint32_t idealAmountToExtend,
	...
	if (surplusWeGot > 8) {
		...
		if (grabResult.amountsExtended[0] > 8) {
			...
			markSpaceAsEmpty((uintptr_t)address + oldAllocatedSize + grabResult.amountsExtended[0] + 8,
