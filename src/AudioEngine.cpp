...
void init() {
	...
	i2sRXBufferPos = (uintptr_t)getRxBufferStart()
...
void routine() {
	...
	int numSamples = ((uintptr_t)(saddr - i2sTXBufferPos) >> (2 + NUM_MONO_OUTPUT_CHANNELS_MAGNITUDE))
...
*/
	...
	if ((midiEngine.anythingInOutputBuffer() || anyGateOutputPending) && !isTimerEnabled(TIMER_MIDI_GATE_OUTPUT)) {
		...
		uintptr_t saddrMovementSinceStart =
...
bool doSomeOutputting() {
	...
	while (renderingBufferOutputPosNow != renderingBufferOutputEnd) {
		...
		if (!(((uintptr_t)((uintptr_t)i2sTXBufferPosNow - saddr) >> (2 + NUM_MONO_OUTPUT_CHANNELS_MAGNITUDE))
		      & (SSI_TX_BUFFER_NUM_SAMPLES - 1))) {
	...
	if (numSamplesOutputted) {
		...
		if (i2sRXBufferPos >= (uintptr_t)getRxBufferEnd())
		...
		for (SampleRecorder* recorder = firstRecorder; recorder; recorder = recorder->next) {
			...
			// Recording from an input source
				...
				uintptr_t stopPos =
				    (i2sRXBufferPos < (uintptr_t)recorder->sourcePos) ? (uintptr_t)getRxBufferEnd() : i2sRXBufferPos;
...
void slowRoutine() {
	...
	uintptr_t latencyWithinAppropriateWindow =
	    (((rxBufferWriteAddr - (uintptr_t)i2sRXBufferPos) >> (2 + NUM_MONO_INPUT_CHANNELS_MAGNITUDE))
